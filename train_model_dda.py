import torch
import numpy as np
from modeling.mygpt import MyGPTModel
...
def plot_loss_curve(x, y):
    plt.plot(x, y)
    plt.title("LLM Training Loss w/ DDA + Decoupled RoPE")
    plt.xlabel("tokens")
    plt.ylabel("cross entropy loss")
    plt.savefig("./figures/dda_decoupled_rope_training_curve.png")
...
def train():
    ...
    model = MyGPTModel(d_model=512, n_heads=16, layers=8, vocab_size=10000,
                     max_seq_len=1024)
    ...
    # save model weights
    torch.save(model.state_dict(), "./weights/dda_weights.pt")
